<template>
  ...
  <div>
    <div class="n-layout-page-header">
      <n-card :bordered="false" title="AED定位">
    ...
    <n-card :bordered="false" class="proCard">
      ...
      <BasicTable  ref="actionRef" openChecked :columns="columns" :request="loadDataTable" :row-key="(row) => row.id" :actionColumn="actionColumn" :scroll-x="scrollX" :resizeHeightOffset="-10000"  :checked-row-keys="checkedIds" @update:checked-row-keys="handleOnCheckedRow">
        <template #tableTitle>
          <n-button type="primary"  @click="addTable" class="min-left-space" v-if="hasPermission(['/aedsLocation/edit'])">
            <template #icon>
              <n-icon>
                <PlusOutlined />
              </n-icon>
          ...
          <n-button type="error" @click="handleBatchDelete" class="min-left-space" v-if="hasPermission(['/aedsLocation/delete'])">
            <template #icon>
              <n-icon>
                <DeleteOutlined />
              </n-icon>
          ...
          <n-button type="primary" @click="handleExport" class="min-left-space" v-if="hasPermission(['/aedsLocation/export'])">
            <template #icon>
              <n-icon>
                <ExportOutlined />
              </n-icon>
...
<script lang="ts" setup>
  ...
  import { List, Export, Delete } from '@/api/aedsLocation';
        ...
        actions: [
          {
            ...
            auth: ['/aedsLocation/edit'],
          },

          {
            label: '删除',
            onClick: handleDelete.bind(null, record),
            auth: ['/aedsLocation/delete'],
          },
        ],
        dropDownActions: [
          {
